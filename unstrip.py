...
def get_so_offsets(sopath):
    result = check_output(['nm', '-D', sopath])
    so_offsets = []
...
def unstrip(fpath, sopath):
    ...
    for k, v in so_offsets:
        ...
        m = re.search(r'^[^\xe8\xe9\xc3]{4,}.', so_data[v:], re.DOTALL)
        if m:
            signature = m.group(0)
        else:
            continue
        try:
            ...
            continue
        print "%s = %s" % (k, hex(offset+index))

if __name__ == '__main__':
    if len(sys.argv) < 3:
        print >>sys.stderr, "Usage: python %s PATH SO_PATH" % sys.argv[0]
        sys.exit(1)
